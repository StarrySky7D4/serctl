...
### 未发布的单人维护治理变更（2026-09-04）

- 按维护者确认，新增明确的 `single-maintainer` 模式：`StarrySky7D4` 可同时负责实测与最终手动批准；不再以虚构的第二账号满足独立审核条件。
- 新接受回执使用 schema v2 并明确记录治理模式，下载计划和最终验证均固定策略与维护者身份；旧 schema v1 默认仍执行独立身份约束，不能由下载的 JSON 自行降级。
- 新发布环境与旧标签环境分离；保留全部五类实机回执、产物字节绑定、OIDC、失败关闭和不可变标签要求。当前 beta.3 未因此通过验收或创建 Release；应用这些治理变更需新的标签提交。
- 修复验收自测中 PowerShell 对 JSON 时间字符串的隐式转换，加入未修改夹具往返成功与单人模式正负例回归，避免无关时间错误掩盖负例。

## v1.0.0-beta.3 - 2026-09-03

> **候选状态（尚未验收/发布）**：当前 workspace 与预发布标记已同步为 `v1.0.0-beta.3`；exact-tag CI、受控实机和仓库治理门禁仍须通过后方可发布。
