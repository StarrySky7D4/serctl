...
function Assert-QualityCheckoutHasFullHistory {
    param(
    ...
    $normalized = [regex]::Replace($quality.Groups['body'].Value, '\s+', ' ').Trim()
    $checkout = (
        'uses: actions/checkout@3d3c42e5aac5ba805825da76410c181273ba90b1 ' +
        '# v7.0.1 with: fetch-depth: 0 persist-credentials: false'
    )
    Assert-TestCondition (
        ([regex]::Matches($normalized, [regex]::Escape($checkout))).Count -eq 1
...
foreach ($requiredAclBoundary in @(
    ...
    'observer_lock_denial_unclassified',
    'observer_write_denial_unclassified',
    'refusing recursive cleanup of reparse-point probe root',
    'observer-parent-control',
    'observer_vault_opened',
    'observer_vault_lock_readable',
    'observer_vault_writable',
    'owner_reopen_passed',
