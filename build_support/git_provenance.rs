...
fn main() {
    ...
    // All shipped binaries include this shared implementation from their tiny
    // crate-local build.rs wrapper. Watch the shared source explicitly so a
    // provenance hardening change invalidates every binary's build identity.
    complete &= emit_rerun_path(&work_dir.join("../../build_support/git_provenance.rs"));
