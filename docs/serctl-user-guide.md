...
命令输出区会同时跟随窗体的可用宽度和高度铺满，不再固定为若干行；缩放窗口时，顶部 profile 区和底部状态区仍保留必要空间。
...
目录与文件名称默认左对齐；列表会随窗体宽度和高度动态铺满，并为上传/下载控制区及当前传输进度卡预留空间。大型目录只实例化当前可见行，避免因窗体放大或刷新而一次布局全部条目。
...
点击启动 Bash 后可交互输入命令，并可发送 `Ctrl+C`、`Ctrl+D` 或清屏。页面采用终端式深色输出区、`$` 提示符和底部输入栏，输出区随窗体宽高动态铺满。这里的“模拟 Bash”仅指桌面呈现方式；底层仍是远端真实 PTY，不会在本机伪造命令结果。关闭 UI、撤销授权或授权到期会终止对应 shell。
